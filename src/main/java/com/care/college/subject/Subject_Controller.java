...
public class Subject_Controller {
   ...
   public String sugang(@RequestHeader(name = HttpHeaders.AUTHORIZATION, required = false) String token,
                  Model model) {
                ...
                for (Integer subject_No : subjectNos) {
                   ...
                   // 현재 수강하려는 강의가 이미 수강한 강의인지 확인
                     if (existingSubjectNos.contains(subject_No)) {
 	                    ...
 	                    int res = service.addToSubjectSchedule(cartList, subjects, student_No, Student_Name);
